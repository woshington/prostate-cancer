...
clean = pd.read_csv("data/without_pen_mask.csv")
clean = clean[clean["reason"]=="marks"].reset_index(drop=True)

dataframe_train = dataframe_train[~dataframe_train['image_id'].isin(clean['image_id'])]
...
class EfficientNet(nn.Module):
    def __init__(self, backbone, output_dimensions):
        super(EfficientNet, self).__init__()
        self.efficient_net = efficientnet_model.EfficientNet.from_pretrained(backbone)
        self.efficient_net.load_state_dict(torch.load(pretrained_model[backbone], weights_only=True))
...
def calculate_metrics(preds, targets, dataframe_valid):
    ...
    quadraditic_weighted_kappa = cohen_kappa_score(preds, targets,
                                                   weights='quadratic')  # Calcula o kappa quadrático ponderado dos dados em geral

    quadraditic_weighted_kappa_karolinska = cohen_kappa_score(preds[dataframe_valid['data_provider'] == 'karolinska'],
                                                              dataframe_valid[dataframe_valid[
                                                                                  'data_provider'] == 'karolinska'].isup_grade.values,
                                                              weights='quadratic')
    quadraditic_weighted_kappa_radboud = cohen_kappa_score(preds[dataframe_valid['data_provider'] == 'radboud'],
                                                           dataframe_valid[dataframe_valid[
                                                                               'data_provider'] == 'radboud'].isup_grade.values,
                                                           weights='quadratic')
...
def validation_step(model, dataset_loader, dataframe_valid, get_logits=False):
    ...
    with torch.no_grad():  # Desativa o cálculo de gradientes
        for index, (batch_data, batch_targets) in enumerate(bar_progress):  # Itera sobre o DataLoader
            ...
            logits = model(
                batch_data)  # Classificação do lote de dados / Retorna o valores brutos dos neurônios (logits)
            ...
            prediction = prediction.sum(
                1).detach().round()  # Realiza a soma do valores de saída, removendo a dependência do grafo computacional (detach)
            ...
            targets.append(batch_targets.sum(
                1))  # realiza a soma dos targets para obter o valor do isup_grade (rotulo real) e salva na lista
    ...
    accuracy, quadraditic_weighted_kappa, quadraditic_weighted_kappa_karolinska, quadraditic_weighted_kappa_radboud = calculate_metrics(
        preds, targets, dataframe_valid)
...
def train_model(fold, model, epochs, optimizer, scheduler, train_dataloader, valid_dataloader, valid_dataframe,
                path_to_save_model):
    ...
    for epoch in range(1, epochs + 1):
        ...
        train_loss = training_step(model, train_dataloader, optimizer, epoch)
        metrics = validation_step(model, valid_dataloader, valid_dataframe)
        ...
        with open('logs/history/fold_03_penmark_certo.txt', 'a') as f:
            f.write(log_epoch + '\n')

        best_metric_criteria = model_checkpoint(model, best_metric_criteria, metrics["quadraditic_weighted_kappa"],
                                                path_to_save_model)

        scheduler.step()


train_indexes = np.where((dataframe_train['fold'] != 3))[0]
valid_indexes = np.where((dataframe_train['fold'] == 3))[0]

dataframe_train_fold = dataframe_train.loc[train_indexes]
dataframe_valid_fold = dataframe_train.loc[valid_indexes]

print(dataframe_train_fold.shape)
print(dataframe_valid_fold.shape)
#
# dataset_train = PandasDataset(images_dir, dataframe_train_fold, transforms=transforms_train)
# dataset_valid = PandasDataset(images_dir, dataframe_valid_fold, transforms=None)
#
...
#
# model = EfficientNet(backbone=backbone_model, output_dimensions=output_classes)
# model = model.to(device)
#
# optimizer = optim.Adam(model.parameters(), lr=init_lr / warmup_factor)
# scheduler_cosine = torch.optim.lr_scheduler.CosineAnnealingLR(optimizer, n_epochs - warmup_epochs)
# scheduler = GradualWarmupScheduler(optimizer, multiplier=warmup_factor, total_epoch=warmup_epochs,
#                                    after_scheduler=scheduler_cosine)
#
# save_path = f'pre-trained-models/fold_03_30_epochs_penmark_certo.pth'
#
# train_model(3, model, n_epochs, optimizer, scheduler, train_dataloader, valid_dataloader, dataframe_valid_fold,
#             save_path)
