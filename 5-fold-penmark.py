...
df_rm_images = pd.read_csv(f"data/without_pen_mask.csv")
df_train = df_train[df_train['image_id'].isin(df_rm_images['image_id']) == False]
...
for i, (train_indexes, valid_indexes) in enumerate(stratified_k_fold.split(df_train, df_train['isup_grade'])):
    df_train.loc[valid_indexes, 'fold'] = i
    
df_train.to_csv(f'data/train_{n_folds}fold.csv', index=False)
...
def train_model(fold, model, epochs, optimizer, scheduler, train_dataloader, valid_dataloader, valid_dataframe, path_to_save_model):
    ...
    for epoch in range(1, epochs + 1):
        ...
        with open('logs/history/remove-penmarks-manual.txt', 'a') as f:
            f.write(log_epoch + '\n')
...
for fold in range(n_folds):
    ...
    train_indexes = np.where((df_train['fold'] != fold))[0] # Pega os índices de treino / Todos os índices que não correspondem ao fold atual são de treino
    valid_indexex = np.where((df_train['fold'] == fold))[0] # Pega os índices de validação

    dataframe_train_fold = df_train.loc[train_indexes]
    dataframe_valid_fold = df_train.loc[valid_indexex]
    ...
    model = EfficientNet(backbone = backbone_model, output_dimensions = output_classes) # Instancia o modelo EfficientNet pré-treinado
    ...
    save_path = f'pre-trained-models/fold-penmark/fold_{fold}.pth'
